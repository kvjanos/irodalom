{
	...
	"inRepository": true,
	"browserSupport": "gcs",
	"lastUpdated": "2012-03-13 17:20:00"
}
...
FW.Scraper({ itemType         : 'magazineArticle',
			 detect           : FW.Xpath('//div[@class="issue"]/a[contains(text(),"Еженедельник")]'),
			 ...
			 creators         : FW.Xpath('//div[contains(@class, "document_authors")][preceding-sibling::div[@class="document_vvodka"]]').text().split(",").cleanAuthor("author"),
			 date             : FW.Xpath('//div[@class="issue"]/a').text().replace(/\n/g," ").replace(/^.*(\d{2})\.(\d{2})\.(\d{4})\s*$/,"$3-$2-$1"),
			 issue            : FW.Xpath('//div[@class="issue"]/a').text().match(/№\s*(\d+(?: \(\d+\))?)/,1),
			 abstractNote     : FW.Xpath('//div[@class="document_vvodka"]').text().trimInternal(),
			 publicationTitle : FW.Xpath('//div[@class="issue"]/a').text().match(/^\s*Еженедельник\s+"([^"]+)"/,1)
});

/** News Articles */
FW.Scraper({ itemType         : 'newspaperArticle',
			 detect           : FW.Xpath('//div[@class="issue"]/a[contains(text(),"Газета") or contains(text(),"Online")  or contains(text(),"FM")]'),
			 title            : FW.Xpath('//div[@class="document"]/div[@class="title"]').text().trim(),
			 attachments      : [
			 { 
				 url : FW.Url(),
		  ...
		  creators         : FW.Xpath('//div[contains(@class, "document_authors")][preceding-sibling::div[@class="document_vvodka"]]')
			 					.text().replace(/\s?отдел\s[^,]*,?/gi,"")
			 					.split(",").trim().cleanAuthor("author"),
			 date             : FW.Xpath('//div[@class="issue"]/a').text().replace(/\n/g," ").replace(/^.*(\d{2})\.(\d{2})\.(\d{4})\s*(?:\/\/\s*\d+:\d+)?$/,"$3-$2-$1"),
			 issue            : FW.Xpath('//div[@class="issue"]/a').text().match(/№\s*(\d+(?: \(\d+\))?)/,1),
			 abstractNote     : FW.Xpath('//div[@class="document_vvodka"]').text().trimInternal(),
			 publicationTitle : FW.Xpath('//div[@class="issue"]/a').text().match(/^[^"]*"([^"]+)"/,1)
});

/** News Articles - second format */
FW.Scraper({ itemType         : 'newspaperArticle',
			 detect           : FW.Url().match(/\/News\//),
			 creators         : FW.Xpath('//div[contains(@class, "document_authors")][preceding-sibling::div[@class="title"]]')
			 					.text().replace(/\s?отдел\s[^,]*,?/gi,"")
			 					.split(",").trim().cleanAuthor("author"),
			 publicationTitle : "Коммерсантъ",
			 title            : FW.Xpath('//table[contains(@class,"news_block")]//div[@class="title"]').text().trim(),
			 attachments      : [
			 { 
				 url : FW.Url(),
				 type : "text/html",
...
FW.MultiScraper({ itemType  : "multiple",
				  detect    : FW.Xpath('//table[contains(@class,"searchresults")]'),
				  choices : {
...
var testCases = [
	...
	{
		"type": "web",
		"url": "http://kommersant.ru/Search/Results?places=1%2C2%2C3%2C5%2C6%2C14%2C17%2C52%2C57%2C61%2C62%2C66%2C84%2C86%2C198%2C210%2C217%2C9999&categories=20&isbankrupt=false&datestart=08.10.2011&dateend=08.11.2011&sort_type=0&sort_dir=0&region_selected=0&results_count=300&saved_query=&saved_statement=&page=1&search_query=%EA%EE%EC%E8%F1%F1%E8%E8",
		"items": "multiple"
	},
	{
		"type": "web",
		"url": "http://www.kommersant.ru/doc/1832739?stamp=634593611705807236",
		"items": [
			{
		...
		"items": [
			{
				...
				"url": "http://www.kommersant.ru/doc/1836636?themeid=589",
				"date": "2011-12-12",
				"publicationTitle": "Коммерсантъ FM",
