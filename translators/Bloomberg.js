{
	...
	"target": "^https?://(www|search.?)?\\.bloomberg\\.com",
	"minVersion": "2.1.9",
	"maxVersion": null,
	"priority": 100,
	"inRepository": true,
...
attachments      : [{ url: FW.Url().replace(/news\//, "news/print/"),
  ...
  type: "text/html" }],
creators         : FW.Xpath('//div[@id="primary_content"]//cite[@class="byline"]/text()').text().replace(/^\s*By/, "").trimInternal().replace(/ -$/, "").split(/ and /).cleanAuthor("author"),
