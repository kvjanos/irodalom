...
FW.Scraper({
itemType         : "blogPost",
detect           : FW.Xpath('//div[@class="components-middle"]'),
attachments      : { url : FW.Url() ,
					 title : "Tweet Snapshot" ,
					 type : "text/html",
					 snapshot: true },
blogTitle		 : FW.Url().match(/([^\/]+)\/status\/\d+/,1).prepend("@"),
creators         : FW.Xpath('//div[@class="components-middle"]//strong[contains(@class,"fullname")]')
						.text().cleanAuthor("author"),
// This part is fragile, since we have to iterate over the attributes
date         : FW.Xpath('//div[@class="components-middle"]//span[@class="metadata"]/span[1]')
						.text(),
websiteType		 : "microblog",
title			 : FW.Xpath('//div[@class="components-middle"]//p[@class="js-tweet-text"]').text(),
hooks            : {
		"scraperDone" : function (item, doc, url) {
				var match = item.date.match(/(\d+:\d+(?: AM| PM)?) - (\d+) ([A-Za-z]+) (\d+)/);
				item.date = (new Date(match[2] + ' ' + match[3] + ' 20' + match[4] + ' ' + match[1])).toGMTString();
			}
}
});
...
var testCases = [
	{
		"type": "web",
		"url": "http://twitter.com/#!/zotero/status/105608278976905216",
		"defer": true,
		"items": [
			{
				"itemType": "blogPost",
				...
				"blogTitle": "@zotero",
				"websiteType": "microblog",
				"date": "Mon, 22 Aug 2011 11:52:00 GMT",
				"title": "Zotero 3.0 beta is now available with duplicate detection and tons more. Runs outside Firefox with Chrome or Safari!  zotero.org/blog/announcin…"
			}
		]
	}
]
