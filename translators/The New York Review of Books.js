...
function detectWeb(doc, url) {
	...
	if ( !Zotero.Utilities.xpathText(doc,
			'//body/*[not(self::iframe) and not(self::script) and not(self::style)]',
...
title            : FW.Xpath('//div[@id="page-title"]/h2').text().trim(), 
creators         : FW.Xpath('//div[@id="page-title"]/h3').text().cleanAuthor("author"), 
url		 : FW.Url().remove(/\?.*/),
attachments      : [
...
date             : FW.Xpath('//div[@id="page-title"]/h5').text(), 
...
title            : FW.Xpath('//div[@id="article-body"]/h2').text().trim(), 
creators         : FW.Xpath('//div[@id="article-body"]/h3').text().cleanAuthor("author"), 
url		 : FW.Url().remove(/\?.*/),
attachments      : [
		{ url : FW.Url().remove(/\?.*/).append("?pagination=false"),
		  type : "text/html",
...
date             : FW.Xpath('//div[@id="article-body"]/p[last()]/em').text(), 
blogTitle        : "NYRblog"
});
...
urls            : FW.Xpath('//ul[@class="issue-article-list"]/li/h3/a').key('href').text() }
});/** BEGIN TEST CASES **/
var testCases = [
	{
		"type": "web",
		...
		"items": [
			{
				...
				"attachments": [
					{
						"url": "http://www.nybooks.com/articles/archives/2011/dec/08/zuccotti-park-what-future/?pagination=false",
						"title": "NYRB Snapshot",
						"type": "text/html"
					}
				],
