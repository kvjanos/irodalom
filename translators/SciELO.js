{
	...
	"target": "^https?://(www\\.)?(socialscience\\.|proceedings\\.|biodiversidade\\.|caribbean\\.|comciencia\\.|inovacao\\.)?(scielo|scielosp)\\.",
	"minVersion": "2.1.9",
	"maxVersion": null,
	"priority": 100,
	"inRepository": true,
	"browserSupport": "gcs",
	"lastUpdated": "2012-03-17 05:45:00"
}
...
attachments : [{ url: FW.Xpath('//meta[@name="citation_pdf_url"]').key("content").text().trim(),
  title: "SCIELO Journal Snapshot",
  type: "application/pdf" }],
//make sure there are no empty authors:
creators : FW.Xpath('//meta[@name="citation_author" or @name="citation_authors"]').key("content").text().replace(/(;[^A-Za-z0-9]*)$/, "").split(";").cleanAuthor("author", useComma="true"),
...
FW.MultiScraper({
itemType : 'multiple',
detect : FW.Url().match(/search\.scielo\.org/),
choices : {
  titles : FW.Xpath('//div[@class="data"]/h3/a').text().trim(),
  urls : FW.Xpath('//div[@class="data"]/h3/a').key("href")
}
});
		...
		"items": [
			{
				...
				"attachments": [
					{
						"url": "http://www.scielosp.org/scielo.php?script=sci_pdf&pid=S0034-89102007000900015&lng=en&nrm=iso&tlng=pt",
						"title": "SCIELO Journal Snapshot",
						"type": "application/pdf"
					}
				],
		...
		"items": [
			{
				...
				"attachments": [
					{
						"url": "http://www.scielo.br/scielo.php?script=sci_pdf&pid=S0104-62762002000200002&lng=en&nrm=iso&tlng=pt",
						"title": "SCIELO Journal Snapshot",
						"type": "application/pdf"
					}
				],
