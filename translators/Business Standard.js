{
	...
	"inRepository": true,
	"browserSupport": "gcs",
	"lastUpdated": "2012-04-01 00:25:00"
}
...
detect           : FW.Xpath('//td/a[@class="link" and contains(text(), "Print this")]'),
title            : FW.Xpath('//title').text().trim(),
attachments      : [{ url: FW.Url().replace(/node/,"print"),
  title:  "Business Standard India Snapshot",
  type: "text/html" }],
creators         : FW.Xpath('//table/tbody/tr/td[contains(@style, "FONT-SIZE: 11px")]').text().match(/.+\//).replace(/\//, "").cleanAuthor("author"),
date             : FW.Xpath('//table/tbody/tr/td[contains(@style, "FONT-SIZE: 11px")]').text().match(/\s*[A-Za-z]{3}\s*\d\d?,\s*\d{4}/),
...
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Url().match(/\/search_/),
choices          : {
  titles :  FW.Xpath('//a[@class="subhead2blue"]').text().trim(),
  urls    :  FW.Xpath('//a[@class="subhead2blue"]').key("href")
}
});/** BEGIN TEST CASES **/
var testCases = [
	{
		"type": "web",
		"url": "http://www.business-standard.com/india/news/air-india-unions-withdraw-strike-threat/469642/",
		"items": [
			{
				"itemType": "newspaperArticle",
				"creators": [
	...
	{
		"type": "web",
		"url": "http://www.business-standard.com/india/news/t-n-ninan-make-believe-solutions/469565/",
		"items": [
			{
				"itemType": "newspaperArticle",
				"creators": [
