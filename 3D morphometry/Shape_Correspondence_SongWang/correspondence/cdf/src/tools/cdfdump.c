...
long startingOffset;
{
  ...
  for (;;) {
     EOFok = TRUE;
     if (offset == fileSize || noMore) return;
     ...
     switch (recordType) {
       ...
       case CPR_: strcpyX (text, "CPR", MAX_SCREENLINE_LEN); noMore = TRUE; break;
     ...
     sprintf (EofS(text), ", %ld bytes (offset: %s)\n",
	      (long) recordSize, FileOffset(offset));
     WriteOut (OUTfp, text);
     offset += recordSize;
     if (!SeekTo(CDFfp,offset,vSEEK_SET)) return;
...
#if !defined(win32)
     sprintf (EofS(text), ", %lld bytes (offset: %s)\n",
              recordSize, FileOffset64(offset));
#else
     sprintf (EofS(text), ", %I64d bytes (offset: %s)\n",
              recordSize, FileOffset64(offset));
#endif
     WriteOut (OUTfp, text);
     offset += recordSize;
...
long startingOffset;
{
  ...
  ****************************************************************************/
  ...
  if (startingOffset == NO_OFFSET) {
    ...
    if (MOST(level)) {
      sprintf (text, "Magic number (1): %s\n", Hex32(int32));
      WriteOut (OUTfp, text);
    }
    if (!Read4(CDFfp,&int32)) return;
    if (MOST(level)) {
      sprintf (text, "Magic number (2): %s\n", Hex32(int32));
      WriteOut (OUTfp, text);
    }
    usedBytes += 8;
  }
  ...
  ****************************************************************************/
  for (;;) {
     ...
     *************************************************************************/
     if (!Read4(CDFfp,&recordType)) return;
     if (MOST(level)) {
       sprintf (text, "RecordType: %ld ", (long) recordType);
       switch (recordType) {
         ...
         case CPR_: strcatX (text, "(CPR)\n", MAX_SCREENLINE_LEN); noMore = TRUE; break;
     ...
     switch (recordType) { 
	  ...
	  if (MOST(level)) {
	    Logical allNULs = TRUE; int i;
            if (!Read4(CDFfp,&int32)) return;
            sprintf (text, "GDRoffset: %s\n", FileOffset(int32));
            WriteOut (OUTfp, text);
            if (!Read4(CDFfp,&version)) return;
            sprintf (text, "Version: %ld\n", (long) version);
            WriteOut (OUTfp, text);
            if (!Read4(CDFfp,&release)) return;
            sprintf (text, "Release: %ld\n", (long) release);
            WriteOut (OUTfp, text);
            if (!Read4(CDFfp,&encoding)) return;
            sprintf (text, "Encoding: %ld\n", (long) encoding);
            WriteOut (OUTfp, text);
            if (!Read4(CDFfp,&int32)) return;
            sprintf (text, "Flags: 0x%lX (", (long) int32);
            sprintf (EofS(text), "%s,",
	  ...
	  if (MOST(level)) {
            if (!Read4(CDFfp,&int32)) return;
            sprintf (text, "rVDRhead: %s\n", FileOffset(int32));
            WriteOut (OUTfp, text);
            if (!Read4(CDFfp,&int32)) return;
            sprintf (text, "zVDRhead: %s\n", FileOffset(int32));
            WriteOut (OUTfp, text);
            if (!Read4(CDFfp,&int32)) return;
            sprintf (text, "ADRhead: %s\n", FileOffset(int32));
            WriteOut (OUTfp, text);
            if (!Read4(CDFfp,&int32)) return;
            fileSize = int32;
            sprintf (text, "eof: %s\n", FileOffset(int32));
            WriteOut (OUTfp, text);
            if (!Read4(CDFfp,&int32)) return;
            sprintf (text, "NumVar: %ld\n", (long) int32);
            WriteOut (OUTfp, text);
            if (!Read4(CDFfp,&int32)) return;
            sprintf (text, "NumAttr: %ld\n", (long) int32);
            WriteOut (OUTfp, text);
            if (!Read4(CDFfp,&int32)) return;
            sprintf (text, "rMaxRec: %ld\n", (long) int32);
            WriteOut (OUTfp, text);
            if (!Read4(CDFfp,&rNumDims)) return;
            sprintf (text, "rNumDims: %ld\n", (long) rNumDims);
            WriteOut (OUTfp, text); 
            if (!Read4(CDFfp,&int32)) return;
            sprintf (text, "NumZvars: %ld\n", (long) int32);
            WriteOut (OUTfp, text); 
            if (!Read4(CDFfp,&int32)) return;
            sprintf (text, "UIRhead: %s\n", FileOffset(int32));
            WriteOut (OUTfp, text); 
            if (!Read4(CDFfp,&int32)) return; 
	  ...
	  if (MOST(level)) {
	    char delim;
            ...
            WriteOut (OUTfp, text);
            if (VDRwastedSpace) {
              Byte wastedSpace[VDR_WASTED_SIZE];
  ...
  ****************************************************************************/
  for (;;) {
     ...
     *************************************************************************/
     ...
     EOFok = TRUE;
     if (noMore || offset == fileSize64) return;
     if (!Read8(CDFfp,&recordSize)) return;
     EOFok = FALSE;
     if (recordSize <= (OFF_T) 0) {
#if !defined(win32)
     ...
     *************************************************************************/
     switch (recordType) {
	  ...
	  if (MOST(level)) {
	    ...
	    if (!Read8(CDFfp,&int64)) return;
#if !defined(win32)
	    sprintf (text, "uSize: %lld\n", int64);
#else
            sprintf (text, "uSize: %I64d\n", int64);
#endif
	    WriteOut (OUTfp, text);
	    if (!Read4_64(CDFfp,&int32)) return;
	    sprintf (text, "rfuA: %ld\n", (long) int32);
