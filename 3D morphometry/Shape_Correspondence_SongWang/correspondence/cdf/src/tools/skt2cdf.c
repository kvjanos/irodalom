  ...
  switch (nextItem) {
    ...
    **************************************************************************/
    case GaVALUEfield: {
      ...
      long eNumElems;
      if (!STRINGdataType(eDataType)) {
	if (!DecodeValues(item,eDataType,&eNumElems,&valuesB,EPOCH0_STYLE)) {
    ...
    **************************************************************************/
	...
	sprintf (msg, "Extra character(s) after record variance (`%s').",
		 &item[1]);
	ParseError (msg);
	return FALSE;
      }
    ...
    **************************************************************************/
    case VARDIMVARYfield:
      if (strlen(item) > (size_t) 1) {
	char msg[MAX_MSG_LEN+1];
	sprintf (msg, "Extra character(s) after dimension variance (`%s').",
		 &item[1]);
	ParseError (msg);
	return FALSE;
      }
    ...
    **************************************************************************/
    case VaeVALUEfield: {
      ...
      long eNumElems;
      if (!STRINGdataType(eDataType)) {
	if (!DecodeValues(item,eDataType,&eNumElems,&eValueB,EPOCH0_STYLE)) {
	  ParseError ("Illegal entry value(s).");
	  return FALSE;
	}
      }
    ...
    **************************************************************************/
    case VARVALUEfield: {
      ...
      long numElems;
      if (!STRINGdataType(vDataType)) {
	if (!DecodeValues(item,vDataType,&numElems,&value,EPOCH0_STYLE)) {
	...
	if (len > vNumElems) {
	  ...
	  sprintf (tempS, "       number %d (this is a non-fatal error).\n",
		   lineN);
	  ...
	  if (len < vNumElems) {
	    if (mLog) {
	      ...
	      WriteOut (stdout, varName);
	      WriteOut (stdout, "'\n");
	      sprintf (tempS, "         at line number %d.\n", lineN);
	      WriteOut (stdout, tempS);
	    }
...
{
  char msg[MAX_MSG_LEN+1];
  sprintf (msg, "Expecting `%s', found `%s' instead.", expected, found);
  ParseError (msg);
  return;
}
...
{
  char msg[MAX_MSG_LEN+1];
  sprintf (msg, "Expecting `%s' or `%s', found `%s' instead.",
...
{
  char msg[MAX_MSG_LEN+1];
  sprintf (msg, "Expecting `%s', `%s', or `%s', found `%s' instead.",
...
char *msg;
{
  ...
  WriteOut (stdout, msg);
  WriteOut (stdout, "\n\n");
  sprintf (tempS, "Error at (or before) line number %d which follows...\n\n",
	   lineN);
  WriteOut (stdout, tempS);
