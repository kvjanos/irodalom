   ...
   ***************************************************************************/
   for (;;) {
	 ...
	 *********************************************************************/
	 ...
	 delim = PickDelimiter (varName, strlen(varName));
	 sprintf (question, "Delete %sVariable %c%s%c ?",
...
struct ItemWindowStruct *IW;
{
   ...
   ***************************************************************************/
   ...
   if (!ReportStatus(status,FALSE)) return FALSE;
   nRecords = maxRec + 1;
   sprintf (IW->label, " CDF \"%s\" %cVariables ", CDFname, BOO(Z,'z','r'));
   if (Z) {
     sprintf (IW->hLines[0], "%ld zVariable%s, ", nVars,
	      (nVars == 1 ? "" : "s"));
     sprintf (&(IW->hLines[0][strlen(IW->hLines[0])]),
   ...
   ***************************************************************************/
   ...
   for (varN = 0; varN < nVars; varN++) {
	...
	CatNcharacters (IW->iLines[(int)varN],
      ...
      ************************************************************************/
      if (Z) {
	sprintf (field, "%ld:[", numDims);
	for (dimN = 0; dimN < numDims; dimN++) {
	   sprintf (&field[strlen(field)], "%s%ld",
		    (dimN == 0 ? "" : ","), dimSizes[dimN]);
	}
	catchrX (field, (int) ']', 0);
	...
	curItem++;
      ...
      * Setup variances.
      ************************************************************************/
      sprintf (field, "%s/", TFvarianceToken(recVary));
      for (dimN = 0; dimN < numDims; dimN++) {
...
Logical *changed;	/* Assumed to be initialized to FALSE by caller. */
{
  ...
  int dimN; void *binary; char delim, *format, varName[CDF_VAR_NAME_LEN256];
  static Logical first = TRUE;
  ...
  ***************************************************************************/
  ...
  for (;;) {
     ...
     *************************************************************************/
     delim = PickDelimiter (varName, strlen(varName));
     sprintf (IW.label, " %cVariable %c%s%c Values ", BOO(Z,'z','r'), delim,
	      varName, delim);
     /*************************************************************************
     ...
     *************************************************************************/
     if (!BuildVarValueLine(Z,recNum,recVary,numDims,indices,
			    ...
			    vvLens,IW.nColsTotal-2,format,&overflow)) {
       ItemWindow (DELETEiw, &IW);
       return FALSE;
     }
     ItemWindow (UPDATEiw, &IW, 0);
		   ...
		   if (IW.itemN == 0) {
		     if (recVary) {
		       ...
		       if (!BuildVarValueLine(Z,recNum,recVary,numDims,indices,
				      ...
				      IW.nColsTotal-2,format,&overflow)) {
				 ItemWindow (DELETEiw, &IW);
				 return FALSE;
		       }
		       ItemWindow (UPDATEiw, &IW, IW.itemN);
		 ...
		 PW.exitChars = exitCharsEdit;
		  ...
		  if (EnterPW(&PW,VARVALUEhelpID)) {
		    ...
		    if (DecodeValues(PW.value,dataType,
				     &nElemsEntered,&newBinary,EPOCH0_STYLE)) {
	   ...
	   *****************************************************************/
	   {
	     ...
	     dimN = IW.itemN - 1;
	     if (dimVarys[dimN]) {
	       sprintf (PW.value, "%ld", indices[dimN]+1);
	       PromptWindow (NEWpw, &PW, (int) strlen(PW.value), LogicalTRUE);
	       for (;;) {
...
Logical BuildVarValueLine (Z, recNum, recVary, numDims, indices, dimVarys,
			   ...
			   vvCols, vvLens, nCols, format, overflow)
Logical Z;
long recNum;
long recVary;
long numDims;
...
Logical *overflow;
{
   ...
   ***************************************************************************/
   vvCols[itemN] = 0;
   if (recVary)
     sprintf (vLine, "%ld", (long) (recNum + 1));
   else
     strcpyX (vLine, "*", 0);
   ...
   ***************************************************************************/
   for (dimN = 0; dimN < numDims; dimN++) {
      ...
      if (dimVarys[dimN])
	sprintf (EofS(vLine), "%ld", (long) (indices[dimN] + 1));
      else
   ...
   *overflow = BOO(STRINGdataType(dataType),
   ...
   EncodeValuesFormat (dataType, numElems, binary, EofS(vLine), format,
		       -BOO(virtual,availCols,0), availCols, EPOCH0_STYLE);
...
Logical center;		/* Center messages displayed? */
{
  ...
  ****************************************************************************/
  delim = PickDelimiter (varName, strlen(varName));
  sprintf (IW->label, " %cVariable %c%s%c `More' ", BOO(Z,'z','r'), delim,
	   varName, delim);
  /****************************************************************************
  * Encode 1st header line.
  ****************************************************************************/
  sprintf (IW->hLines[0], "  DataSpec: %s/%ld", DataTypeToken(dataType),
	   numElems);
  ...
  ****************************************************************************/
  sprintf (IW->hLines[1], "   Records: %ldn/%ldx", nRecords, maxRec + 1);
  ...
  ****************************************************************************/
  ...
  sprintf (EofS(IW->hLines[3]), "   Compression: %s",
	   CompressionToken(cType,cParms));
  if (cType != NO_COMPRESSION && cPct > 0) {
    sprintf (EofS(IW->hLines[3]), " (%ld%%)", cPct);
  }
  /****************************************************************************
  ...
  ****************************************************************************/
  if (CDFformat == SINGLE_FILE) {
    ...
    sprintf (IW->hLines[5], " Indexing: n/a");
  return TRUE;
}
...
Logical *created;       /* Set to TRUE if a variable is created. */
{
  ...
  ****************************************************************************/
  ...
  sprintf (labelName, " New %cVariable ", BOO(Z,'z','r'));
  ...
  delim = PickDelimiter (varName, strlen(varName));
  sprintf (labelOthers, " New %cVariable %c%s%c ", BOO(Z,'z','r'),
	   delim, varName, delim);
  /****************************************************************************
...
Logical *changed;	/* Assumed to be initialized to FALSE by caller. */
{
  ...
  if (useFormat) {
    ...
    format = NULL;
  EncodeValuesFormat (dataType, numElems, binary, PWt.value, format, 0, 0,
		      EPOCH0_STYLE);
  cdf_FreeMemory (binary, FatalError);
  if (format != NULL) cdf_FreeMemory (format, FatalError);
