  ...
  switch (argC) {
	...
	if (dstEncoding != SOURCEencoding) {
	  ...
	  static long encodings[] = {
	    ...
	    MAC_ENCODING, HP_ENCODING, NeXT_ENCODING, ALPHAOSF1_ENCODING,
...
char *dstPath;
{
  ...
  long format, majority, encoding, version;
...
Logical *done;
{
  ...
  *p2 = strchr (*p1, ',');
  ...
  sToken = *p1;
  if (!strcmpIgCase(sToken,"sRecords.PAD")) {
    *sRecordsType = PAD_SPARSERECORDS;
    return TRUE;
  }
  if (!strcmpIgCase(sToken,"sRecords.PREV")) {
    *sRecordsType = PREV_SPARSERECORDS;
    return TRUE;
  }
  if (!strcmpIgCase(sToken,"sRecords.NO")) {
...
Logical *done;
{
  ...
  *p2 = strchr (*p1, ',');
  ...
  if (!strcmpIgCase(cToken,"ahuff.0")) {
  ...
  for (level = 1; level <= 9; level++) {
     ...
     if (!strcmpIgCase(cToken,gzipToken)) {
