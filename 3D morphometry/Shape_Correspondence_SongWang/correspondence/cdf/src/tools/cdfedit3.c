...
#endif
{
  ...
  if (lineS == NULL) {
    ...
    EW.eText = (char *) cdf_AllocateMemory (textLength + 1, FatalError);
    MakeNUL (EW.eText);
...
Logical attr;   /* TRUE: attribute, FALSE: variable. */
{
  ...
  sprintf (lineS[1], "  %c%s%c  ", delim, name, delim);
...
Logical SelectEncoding () {
  ...
  static char *items[] = {
    ...
    "NETWORK   VAX   IBMRS   MAC   NeXT   ALPHAVMSd   ALPHAOSF1"
  };
...
char *label;            /* If NULL, no label. */
{
  ...
  static int exitChars[] = { ENTERkey_FSI, EXITkey_FSI, HELPkey_FSI, NUL };
  static struct ItemWindowStruct IWt = {
    4, 1, 78, NULL, 1, header, 3, items, 15, iLineNs, iCols, iLens, 3, 1,
    trailer, exitChars, REFRESHkey_FSI, FALSE
  };
  static Logical first = TRUE; int itemN;
  char selectedDataType[11+1], delim = ' ', *selected = NULL, *tmpStr;
  if (first) {
    EncodeKeyDefinitions (1, trailer, ENTERkey_FSI, EXITkey_FSI,
			  HELPkey_FSI);
    first = FALSE;
