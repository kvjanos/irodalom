  ...
  default:
    ...
    **************************************************************************/
	...
	if (qop->qualEntered[VALUESqual]) {
	  ...
	  if (!strcmpIgCase(qop->qualOpt[VALUESqual],"none"))
	    varValues = NOvalues;
	  else if (!strcmpIgCase(qop->qualOpt[VALUESqual],"nrv"))
	    varValues = NRVvalues;
	  else if (!strcmpIgCase(qop->qualOpt[VALUESqual],"rv"))
	    varValues = RVvalues;
	  else if (!strcmpIgCase(qop->qualOpt[VALUESqual],"all"))
	    varValues = ALLvalues;
	  else {
	    varValues = NAMEDvalues;
 	    variables = cdf_AllocateMemory (len + 1, FatalError);
...
if (numDims > 0) {
  ...
  for (dimN = 0; dimN < numDims; dimN++) {
     sprintf (&sizes[strlen(sizes)], "%ld ", dimSizes[dimN]);
  }
...
sprintf (text, "%ld/%ld", numVars, numZvars);
Ncharacters (SKTfp, MaxInt((int)(6 - strlen(text)),0), (int) ' ');
sprintf (tempS, "%s     %7ld       %7ld     %5ld/z    %3ld",
  ...
  for (attrN = 0; attrN < numAttrs; attrN++) {
	 ...
	 ccc += WriteOut (SKTfp, "------      ----       -----");
       ...
       ***********************************************************************/
       ...
       WriteOut (SKTfp, "\n\n");
       sprintf (tempS, "  %c%s%c", delim, attrName, delim);
	  ...
	  if (status != NO_SUCH_ENTRY) {
	    ...
	    sprintf (tempS, "%*ld:    %s   ", ENTRY_NUM_WIDTH, entryN + 1,
...
WriteOut (SKTfp, "\n");
sprintf (tempS, "  %c%s%c", delim, varName, delim);
ccc = WriteOut (SKTfp, tempS);
...
sprintf (tempS, "%s %*ld   ", DataTypePaddedString(varDataType),
...
if (zVar) {
  sprintf (tempS, " %*ld ", zVAR_NUMDIMS_WIDTH, numDims);
  WriteOut (SKTfp, tempS);
  Ncharacters (SKTfp, 2, (int) ' ');
  Ncharacters (SKTfp, FRONTpadVALUE("Sizes",sizes), (int) ' ');
  WriteOut (SKTfp, sizes);
  ...
  for (recN = 0; recN <= maxRec; recN++) {
     ...
     for (i = 0; i < nRecValues; i++) {
	  ...
	  return FALSE;
        ...
        ccc = WriteOut (fp, "\n    ");
	if (recVary) {
	  sprintf (tempS, "%ld:[", recN + 1);	  
	}
	else
	  strcpyX (tempS, "[", MAX_LINE_LEN);
        if (numDims > 0) {
          for (dimN = 0; dimN < numDims; dimN++) {
             sprintf (EofS(tempS), "%ld", indices[dimN] + 1);
	...
	ccc += WriteOut (SKTfp, tempS);
        if (STRINGdataType(dataType)) {
          ...
          EncodeValuesFormat (dataType, 1L, value, Evalue,
			      format, 0, 0, EPOCH0_STYLE);
