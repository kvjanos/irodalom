...
static double epoch[2], epochOut[2], mmm;
static double epochHyper[4][2], epochHyperOut[4][2];
...
encodeEPOCH16_x (epoch, "<dom.02>-<month>-<year> <hour>:<min>:<sec>.<fos>",
                 encodeText);
printf("encodeEPOCH16_x: %s\n",encodeText);
encodeEPOCH16_x (epoch, 
                 "<dom.02>-<month>-<year> <hour>:<min>:<sec>.<msc>:<usc>:<nsc>:<psc>",
                 encodeText);
printf("encodeEPOCH16_x: %s\n",encodeText);

   status = CDFlib (PUT_, zVAR_DATA_, epoch,
		    NULL_);
...
******************************************************************************/
   ...
   status = CDFlib (GET_, zVAR_DATA_, epochOut,
		    NULL_);
   if (status < CDF_OK) QuitCDF ("9.1z", status);

   EPOCH16breakdown (epoch, &yearOut, &monthOut, &dayOut, &hourOut, 
...
for (x0= 0; x0 < 4; x0++) {
  ...
  epochHyper[x0][0] = epoch[0];
  epochHyper[x0][1] = epoch[1];
...
status = CDFlib (SELECT_, zVAR_RECNUMBER_, 1L,
		 ...
		 PUT_, zVAR_HYPERDATA_, epochHyper,
		 NULL_);
...
status = CDFlib (GET_, zVAR_HYPERDATA_, epochHyperOut,
		 NULL_);
...
/*
   ...
   status = CDFlib (GET_, zVAR_DATA_, epochOut,
                    NULL_);
*/
  EPOCH16breakdown (epochHyperOut[x0], &yearOut, &monthOut, &dayOut, &hourOut, 
		    &minuteOut, &secondOut, &msecOut, &usecOut, &nsecOut,
			&psecOut);
