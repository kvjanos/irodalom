...
Logical StrStrIgCaseX PROTOARGs((
  char *string, char *chkstring
));
...
int flag;
{
  ...
  /*
   * Set setFileBackward flag. So, if the environment variable is detected
   * to create backward files (and thus calling CDFsetFileBackward2 
   * function), we can ignore it as calling this CDFsetFileBackward 
   * function has precedence over the environment variable approach.
   */
...
* function has been called and as it has precedence
* over the environment variable, calling this function will have no effect at
* all. 
******************************************************************************/
...
int flag;
{
  /* This function is called if the environment variable is set.
   * However, if setFileBackwardFlag flag is already set by calling 
   * CDFsetFileBackward function, this function call is then ignored as
   * CDFsetFileBackward has a precedence over the environment variable. 
   */
  if (setFileBackwardFlag == 0) {
    if (flag != 0) backward = 1;
    else backward = 0;
  }
}
...
/******************************************************************************
* CDFcheckFileBackwardisSet.
*   Checks whether the file backward flag has been set by CDFsetFileBackward 
*   function.
******************************************************************************/

VISIBLE_PREFIX int CDFcheckFileBackwardisSet ()
{
  return setFileBackwardFlag;
}
...
Logical StrStrIgCaseX (string, chkstring)
char *string;
char *chkstring;
{
