...
static void encodeEPOCH16x2 PROTOARGs((
  double epoch, char *encoded
));
static void encodeEPOCH16x3 PROTOARGs((
  double epoch[], char *encoded
));
...
double epoch[2];
{
  ...
  epoch[1] = msec * pow(10.0, 9) + usec * pow(10.0, 6) + nsec * pow(10.0, 3) + 
	     psec;
  return (double) 0.0;
}
...
double epoch[2];
{
  ...
  if (sscanf(temp,"%4ld%2ld%2ld.%7ld%8ld",
     ...
     return ILLEGAL_EPOCH_VALUE;
  fraction = ((double) fractionL1 * pow(10.0, 8) + (double) fractionL2) * 
	     pow(10.0, -15);
  hour = (long) (fraction * 24.0);
  fraction -= (double) (hour / 24.0);
  minute = (long) (fraction * 1440.0);        
  ...
  fraction -= (double) (second / 86400.0);
  /*
  mmm = fraction * (double) 86400.0;
  msec = (long) (mmm * 1000.0);
  mmm -= (double) (msec / 1000.0);
  usec = (long) (mmm * 1000.0);
  mmm -= (double) (usec / 1000.0);
  nsec = (long) (mmm * 1000.0);
  mmm -= (double) (nsec / 1000.0);
  psec = (long) (mmm * 1000.0);
  */
  mmm =  computeEPOCH (year, month, day, hour, minute, second, 0L);
  if (mmm == ILLEGAL_EPOCH_VALUE) return ILLEGAL_EPOCH_VALUE;
  epoch[0] = mmm / (double) 1000.0;
  epoch[1] = fraction * 86400.0 * pow(10.0, 12);
  return (double) 0.0;
}
...
double epoch[2];
{
  ...
  if (mmm == ILLEGAL_EPOCH_VALUE) return ILLEGAL_EPOCH_VALUE;
  epoch[0] = mmm / (double) 1000.0;
  epoch[1] = (double) psec + (double) nsec * pow(10.0, 3) + 
	     (double) usec * pow(10.0, 6) + (double) msec * pow(10.0, 9);
  return (double) 0.0;
}
...
char epString[EPOCH16_3_STRING_LEN+1];
{
  ...
  encodeEPOCHx (epoch[0]*1000.0, 
		"<year>-<mm.02>-<dom.02>T<hour>:<min>:<sec>.",
                tmp);
  strcpyX(epString, tmp, 20);
  encodeEPOCH16x2 (epoch[1], tmp);
  strcpyX(epString+20, tmp, 15);
  epString[35] = 'Z';
...
static void encodeEPOCH16x2 (epoch, encoded)
double epoch;
char *encoded;
{
  ...
  sprintf(encoded, "%3.3ld.%3.3ld.%3.3ld.%3.3ld", msec, usec, nsec, psec);
  encoded[15] = NUL;
  return;
}

static void encodeEPOCH16x3 (epoch, encoded)
double epoch[2];
char *encoded;
{
  char tmp[16+1];
  ...
  mmm1 = ((double) hour * 3600.0 + (double) minute * 60.0 + (double) second) /
	  86400.0; 
  mmm2 = ((double) msec * pow(10.0, 9) + (double) usec * pow(10.0, 6) + 
	  (double) nsec * pow(10.0, 3) + (double) psec) / 
	  (86400.0 * pow(10.0, 12));
  mmm1 = mmm1 + mmm2;
  sprintf(tmp, "%.15f", mmm1);
  strcpyX (encoded, tmp+2, 15);
  encoded[16] = NUL;
  return;
}
...
double epoch[2];
{
  ...
  ****************************************************************************/
  epoch[0] = 86400.0 * (double) daysSince0AD + 3600.0 * (double) hour + 
             60.0 * (double) minute + (double) second;
  epoch[1] = (double) psec + (double) nsec * pow(10.0, 3) + 
             (double) usec * pow(10.0, 6) + (double) msec * pow(10.0, 9);
  return (double) 0.0;
}
...
long *msec, *usec, *nsec, *psec;
{
  ...
  epoch[0] = MINIMUM ((double)MAX_EPOCH_BINARY, epoch[0]);
  ...
  *second = (long) fmod (second_AD, (double) 60.0);
  ...
  if (epoch[1] < 0.0) epoch[1] = -epoch[1];
    epoch[1] = MINIMUM ((double)MAX_EPOCH_BINARY, epoch[1]);
