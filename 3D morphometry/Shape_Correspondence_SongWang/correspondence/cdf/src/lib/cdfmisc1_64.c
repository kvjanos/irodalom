    ...
    **************************************************************************/
    if (CDF->rVars != NULL) {
      for (varN = 0; varN < CDF->NrVars; varN++) {
         ...
         if (Var != NULL) {
           if (Var->fp != NULL) {
             if (!CLOSEv64(Var->fp,NULL)) sX (VAR_CLOSE_ERROR, &pStatus);
             Var->fp = NULL;
           }
         }
      }
...
Logical deleteCDF;      /* Delete CDF file(s)? */
{
  ...
  ****************************************************************************/
  if (deleteCDF) {
    ...
    **************************************************************************/
    if (CDF->dotFp != NULL) V_close64 (CDF->dotFp, NULL);
    if (CDF->uDotFp != NULL) V_close64 (CDF->uDotFp, NULL);
    CloseVarFiles64 (CDF);
  }               
  /****************************************************************************
