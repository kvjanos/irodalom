...
#define HYPER_READ(id,zVar,hyper,buffer) \
...
#define HYPER_WRITE(id,zVar,hyper,buffer) \
CDFlib (SELECT_, CDF_, id, \
		 BOO(zVar,zVAR_RECNUMBER_, \
			  rVARs_RECNUMBER_), hyper.recNumber, \
		 BOO(zVar,zVAR_RECCOUNT_, \
...
void EncodeIndicesJustify PROTOARGs((
  char *string, long numDims, long indices[], int minWidth
));
...
int strncmpIgCasePattern PROTOARGs((char *string1, char *string2, 
  size_t count
));
...
Logical HyperStartOfRecord PROTOARGs((
  struct HyperStruct *hyper, long numDims
));
void WriteOutPct PROTOARGs((int pct));
Logical SwitchMajority PROTOARGs((
  Byte *buffer, Logical rowMajor, long numDims, long dimSizes[],
