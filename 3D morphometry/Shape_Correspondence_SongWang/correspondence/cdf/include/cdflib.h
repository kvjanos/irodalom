...
# if defined(CDFLIB)
    ...
    uInt vMemMask = DEFAULT_vMEMMASK;
# else
    extern Logical useVmem;
    extern long vMemSize;
    extern uInt vMemMask;
# endif
#endif
...
*                                 IBM-PC (MS-DOS, Linux, Solaris),
*                                 DEC Alpha (OSF/1, OpenVMS, POSIX Shell),
******************************************************************************/

#if defined(sun)||defined(MIPSEB)||defined(IBMRS)||defined(HP)||defined(NeXT)||defined(mac)||defined(PPC)||defined(macosX)
...
*                 Solaris), DEC Alpha (OSF/1, OpenVMS, POSIX Shell).
******************************************************************************/
...
struct AllocStruct {
  ...
  struct {
    Logical new;        /* TRUE: Don't append to an existing VVR (even if
...
struct VarStruct {
  Int32 VDRoffset;              /* Byte offset of rVDR/zVDR using 32-bit
				   offset. */
  OFF_T VDRoffset64;            /* Byte offset of rVDR/zVDR using 62-bit
				   offset. */
  ...
  uLong accessed_at;            /* Pseudo-clock time of last access. */
...
struct CDFstruct {
  uInt32 magic;				/* Magic number stored in CDF
					   ...
					   specified. */
  Logical largeFile;                    /* TRUE: if 64-bit offset,
  ...
  vFILE *dotFp;                         /* File pointer to dotCDF file. */
  ...
  uLong pseudo_clock;                   /* Tick count used to determine which
