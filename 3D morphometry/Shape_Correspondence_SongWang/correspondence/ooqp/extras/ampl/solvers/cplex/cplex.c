...
#define Round		mint_val[15].val
 ...
 static char cplex_version[] = "\nAMPL/CPLEX(20010531)\n";
 static char *algname, *baralgname, *endbas, *endtree, *endvec, *logfname;
 static char *startbas, *starttree, *startvec, *workfiledir, *wrtfname;
 static char *version;
...
sf_dbl(Option_Info *oi, keyword *kw, char *v)
{
 ...
 static char **file_name[9] = { &endbas, &endtree, &startbas, &starttree,
...
#define VP (Char*)
 ...
 static keyword keywds[] = {	/* must be in alphabetical order */
	...
	{ "limitperturb",sf_int,	VP CPX_PARAM_PERLIM },
	...
	{ "mipordertype",sf_int,	VP CPX_PARAM_MIPORDTYPE },
...
amplin(ASL *asl, cpxlp **pcpx, FILE **nl, dims *d, int *nintp, char **av)
{
	...
	*pcpx = cpx;
	...
	nbas = n;
	cname = rname = 0;
	if (startbas || endbas || startvec || endvec || wrtfname
 ...
 have_ii:
		...
		if (solinfo[stat].wantobj || endbas) {
			...
			if (!relax && Round >= 0) {
				nint = niv + nbv;
				x2 = x + n_var - nint;
				if (nround = xround(x2, nint, Round & 1, w)) {
