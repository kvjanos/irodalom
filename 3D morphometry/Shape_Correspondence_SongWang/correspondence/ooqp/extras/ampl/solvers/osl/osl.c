...
#define VP (Char*)
 ...
 static Option_Info Oinfo = {"osl", "OSL 2.x", "osl_options",
...
getversion(VOID)
{
	static fint I0=0, I1=1, I2=2, I259=259, I6=6, I7=7, Im1=-1;
	fint rc;
	char *s, *t;
	...
	chk("ekkmset");
	ekkmset_(&rc, dspace, &I6, &I0, &Im1, &I0, &I1, &I259, &I2);
	chk("ekkmset");
	free(dspace);
	...
	if (rc = s - versbuf) {
		*s = 0;
		...
		if (t > s) {
			...
			*++t = 0;
			*s = ' ';
			rc = strlen(osl_vers+1);
			s = Oinfo.version = (char*)Malloc(rc + t - versbuf + 6);
			strcpy(s, osl_vers+1);
			strcpy(s+rc-1, ", OSL ");
			strcpy(s+rc+5, versbuf);
			}
		}
	}
